...
abstract contract ERC721URIStorage is ERC721Enumerable {
    ...
    function _setTokenURI(uint256 tokenId, string memory _tokenURI) internal virtual {
        ...
        // Change: we enforce changing a token URI only once
        require(bytes(_tokenURIs[tokenId]).length == 0, "ERC721URIStorage: URI already set for this token");
...
contract PackBrew is ERC721URIStorage, Ownable {
    ...
    // Free packs: 15360-15759 (400 items)
    uint256 public constant MAX_NFT_SUPPLY = 15760;
    
    // 400 packs set aside to be given as gifts => 8000 total
    uint256 public constant MAX_PACKS_1CARD = 7600;
    ...
    uint256 public MAX_FREE_PACKS = 15760; // can be updated during initial migration
    ...
    function initialMigrateConsumerCard(address minter, uint256 index) external onlyDisbursement {
      ...
      if (i+1 > _mintIndexConsumers) {
        _mintIndexConsumers = i.add(1);
      }
      
      // Consumer receives 1x free pack
      tryMintFreePack(minter);
      MAX_GIFT_FREE_PACK--;
      MAX_GIFT_LUCHADOR--;
      // These free packs must not be taken from regular pack rewards
      MAX_FREE_PACKS++;
      _totalPacks1Card++;
    }
    ...
    function initialMigrateDistributorCard(address minter, uint256 index) external onlyDisbursement {
      ...
      // Distributor receives 3x free pack
      tryMintFreePack(minter);
      tryMintFreePack(minter);
      tryMintFreePack(minter);
      MAX_GIFT_FREE_PACK -= 3;
      MAX_GIFT_LUCHADOR -= 3;
      // These free packs must not be taken from regular pack rewards
      MAX_FREE_PACKS += 3;
      _totalPacks1Card += 3;
    ...
    function getPackBonus(address caller) private returns (uint256) {
      ...
      // returns packsLeft+1 if gift was assigned but all gifts were sold already
      return 0;
    }
